...
python3 Update_CheatSheets_Index.py
# python Generate_RSS_Feed.py

echo "Step 3/7: Create the expected MkDocs folder structure."

cp ../mkdocs.yml $WORK/.
cp ../Preface.md $WORK/documentation/index.md
mv News.xml $WORK/documentation/.
cp -r ../documentation $WORK/documentation/documentation
cp -r ../assets $WORK/documentation/assets
cp ../Index.md $WORK/documentation/Glossary.md
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    # MacOS
    # sed -i '' "$WORK/documentation/index.md"
    # # MacOS
    # sed -i '' "1i\\
    #     Title: Introduction\\
    #     " "$WORK/documentation/index.md"
    sed -i '' 's/Index.md/Glossary.md/g' "$WORK/documentation/Glossary.md"
    # sed -i '' "1i\\
    ...
    sed -i 's/Index.md/Glossary.md/g' $WORK/documentation/Glossary.md
    # sed -i "1iTitle: Index Alphabetical\n" $WORK/documentation/Glossary.md
fi

echo "Step 4/7: Inserting markdown metadata."
for fullfile in "$WORK"/documentation/documentation/*.md
do
